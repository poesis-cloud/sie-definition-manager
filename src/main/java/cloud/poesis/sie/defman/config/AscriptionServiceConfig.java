...
/**
 * Builds the service registry bean that maps each {@link DefinitionSubjectType}
 * to its {@link
 * AbstractAscriptionService} implementation. Spring auto-collects all concrete
 * {@code
...
public class AscriptionServiceConfig {
   ...
   */
  @Bean
  public Map<DefinitionSubjectType, AbstractAscriptionService<? extends AscriptionEntity>> ascriptionServiceRegistry(
      List<AbstractAscriptionService<? extends AscriptionEntity>> services) {
    Map<DefinitionSubjectType, AbstractAscriptionService<? extends AscriptionEntity>> map = new EnumMap<>(
