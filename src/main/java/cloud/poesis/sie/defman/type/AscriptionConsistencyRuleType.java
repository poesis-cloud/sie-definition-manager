...
public enum AscriptionConsistencyRuleType implements RuleType {
    ...
    MECHANISM_RULE_TRIGGER_AS_FIRST_STATEMENT(
            ...
            "A Mechanism rule must begin with exactly one sys.receive(\"<Archetype URI>\") "
    ...
    MECHANISM_RULE_TRIGGER_AS_UNIQUE_STATEMENT(
            ...
            "A Mechanism rule must contain exactly one sys.receive() trigger declaration "
                    + "— multiple sys.receive() calls are forbidden."),

    MECHANISM_RULE_TRIGGER_ARGUMENT_AS_ARCHETYPE_ID(
            "gsm:rules/mechanism/rule/trigger-argument-as-archetype-id",
            "Mechanism rule trigger argument as Archetype URI",
            "sys.receive() must receive exactly one positional argument: a non-empty "
                    + "string literal that resolves to a governed Archetype URI "
    ...
    MECHANISM_RULE_SYS_FLUENT_API_ARITY(
            ...
            "sys.effect() requires 1-2 positional arguments (archetype "
                    + "$id, optional data payload). sys.receive() requires "
                    + "exactly 1 positional argument (Archetype URI). "
                    + "Chain methods .by(), .receive(), .on() each require "
                    + "exactly 1 positional argument (Archetype URI as "
                    + "string literal)."),
    ...
    NORM_APPLICABILITY_ARCHETYPE_REFERENCE_RESOLUTION(
            ...
            "Every ref(\"$id\") root in a Norm applicability expression must contain a "
                    + "grammar-conformant full Archetype URI that resolves to one governed "
                    + "Archetype Ascription."),
    ...
    ARCHETYPE_ALLOF_PROPERTY_TYPE_STABILITY(
            "gsm:rules/archetype/allof/property-type-stability",
            "Archetype allOf property type stability",
            "A host Archetype must not redefine an inherited or composed property with a different "
                    + "explicit JSON Schema type set. Equal type sets and same-type narrowing remain valid."),
    ...
    ARCHETYPE_REF_INTEGRITY(
            "gsm:rules/archetype/ref/integrity",
            "Archetype $ref integrity",
            "Every grammar-valid external gsmarc:// $ref must resolve by its exact full identity to one "
                    + "governed Archetype Ascription."),

    ARCHETYPE_ID_GRAMMAR(
            "gsm:rules/archetype/id/grammar",
            "Archetype ID grammar",
            "An Archetype statement must declare a root $id matching the normative gsmarc identity "
                    + "grammar."),

    ARCHETYPE_ID_TITLE_COHERENCE(
            "gsm:rules/archetype/id/title-coherence",
            "Archetype ID title coherence",
            "The title segment of an Archetype statement $id must exactly equal its title."),

    ARCHETYPE_ID_ROOT_EXCLUSIVITY(
            "gsm:rules/archetype/id/root-exclusivity",
            "Archetype ID root exclusivity",
            "An Archetype statement may declare $id only at the document root."),
    ...
    ASCRIPTION_ARCHETYPE_BASED_ON_GSM_ARCHETYPE(
            ...
            "An Ascription's typing archetype (archetype_id) must reference "
                    + "an Archetype whose top-level $ref chain converges to a GSM base "
    ...
    ASCRIPTION_ARCHETYPE_IN_EFFECT(
            "gsm:rules/ascription/archetype/in-effect",
            "Ascription archetype in effect",
            "A typing Archetype must be ACTIVE or DEPRECATED before a new Ascription may reference it."),
    ...
    ASCRIPTION_PROPERTY_UNIQUENESS_ACROSS_DEFINITIONS(
            ...
            "Identity properties (Structure.purpose and Mechanism.function) and "
                    + "$gsm:unique-annotated properties must be "
    ...
    ARCHETYPE_STEM_UNIQUENESS_ACROSS_DEFINITIONS(
            "gsm:rules/archetype/stem/uniqueness-across-definitions",
            "Archetype stem uniqueness across definitions",
            "An Archetype identity stem is permanently owned by one Definition across all lifecycle "
                    + "statuses; later versions of that identity family must use the same Definition."),

    ARCHETYPE_URI_RESOLUTION_UNIQUENESS(
            "gsm:rules/archetype/uri/resolution-uniqueness",
            "Archetype URI resolution uniqueness",
            "An Archetype URI resolves to at most one governed Ascription (version > 0); rival authoring "
                    + "candidates with version 0 may converge on the same URI."),
