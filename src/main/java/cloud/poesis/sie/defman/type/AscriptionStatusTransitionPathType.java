 ...
 *
 * <p>Each value encodes a named transition edge: a (from, to) pair of {@link AscriptionStatusType}
 * values. The {@code from} field is {@code null} for the initial creation transition.
 *
...
public enum AscriptionStatusTransitionPathType {
  ...
  public static boolean isValid(AscriptionStatusType from, AscriptionStatusType to) {
    return Arrays.stream(values()).anyMatch(p -> p.from == from && p.to == to);
  }
