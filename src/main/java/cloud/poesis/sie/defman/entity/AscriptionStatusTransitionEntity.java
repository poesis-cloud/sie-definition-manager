 ...
 * setters exposed. {@code
 * id} and {@code timestamp} are DB-generated via column defaults
 * ({@code uuid_v7()} and {@code
 * clock_timestamp()}).
 *
 * <p>
 ...
 * validates that {@code
 *       ascription_id} references an existing ascription row across all 9 class
 * tables
 * <li>{@code tgf_sync_ascription_status} — AFTER INSERT: cascades
 * {@code post_status} to the
 ...
 * <li>{@code tgf_reject_transition_mutation} — BEFORE UPDATE/DELETE: blocks any
 * mutation (rows
 * are append-only)
 * </ul>
 *
