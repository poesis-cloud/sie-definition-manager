 ...
 *
 * <p>No materialized columns from {@code statement} — the schema is a JSON document inside the
 * JSONB payload. Identity uniqueness ({@code schema.title}) is enforced via a partial expression
 * index, not a dedicated column.
 *
 * <p>Carries the standard 6-trigger set on the {@code archetype} table (see {@link
...
public class ArchetypeEntity extends AscriptionEntity {
   ...
   * @param archetype the typing archetype (self-referencing for the seed archetype)
