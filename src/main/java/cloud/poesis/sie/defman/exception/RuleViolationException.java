 ...
 *
 * <p>
 * The {@link RuleType} carries all semantic weight (which rule was
 * violated); this class is a thin carrier of {@code ruleType} + contextual
 * {@code site} data describing the violating input instances.
 * </p>
 *
 * <p>
 * Depending on the {@code RuleType}, the API layer maps instances of
 * this exception to HTTP {@code 400 Bad Request} or {@code 409 Conflict}
 * responses, rendered as RFC 9457 {@code ProblemDetail}.
 * </p>
 *
...
public class RuleViolationException extends RuntimeException {
     ...
     * @param site     contextual key-value pairs describing the violating
     *                 input instances; may be {@code null} (treated as empty)
     */
     ...
     */
    public RuleViolationException(RuleType ruleType, String detail, Map<String, Object> site, Throwable cause) {
        super(detail, cause);
        this.ruleType = ruleType;
        this.site = site != null ? Map.copyOf(site) : Map.of();
    }
     ...
     *
     * @return the {@link RuleType} identifying the specific rule that was
     *         violated; never {@code null}
     */
    ...
    /**
     * Returns the GSM rule type URI — a stable, machine-readable identifier
     * mapped to the {@code type} field of RFC 9457 {@code ProblemDetail}.
     *
    ...
    /**
     * Returns a short, human-readable summary of the rule category, mapped
     * to the {@code title} field of RFC 9457 {@code ProblemDetail}.
     *
    ...
    /**
     * Returns structured extension properties for the RFC 9457
     * {@code ProblemDetail} response.
     *
     * <p>
     * The returned map merges the {@linkplain #getSite() site} entries with
     * the {@code rule} name and {@code ruleDescription}.
     * </p>
     *
    ...
    /**
     * Convenience factory that builds the site map from alternating key-value
     * pairs, omitting entries whose value is {@code null}.
     *
     * @param ruleType          the violated GSM rule
     * @param detail            human-readable description of the violation
     * @param siteKeyValuePairs alternating {@code String} keys and
     *                          {@code Object} values; must have an even length
     * @return a new {@code RuleViolationException}
     * @throws IllegalArgumentException if the varargs array has odd length
     */
    public static RuleViolationException of(RuleType ruleType, String detail,
            Object... siteKeyValuePairs) {
        return new RuleViolationException(ruleType, detail, toSite(siteKeyValuePairs));
    }

    /**
     * Convenience factory with cause that builds the site map from alternating
     * key-value pairs, omitting entries whose value is {@code null}.
     *
     * @param ruleType          the violated GSM rule
     * @param detail            human-readable description of the violation
    ...
    /**
     * Converts alternating key-value pairs into an immutable map, omitting
     * entries whose value is {@code null}.
     *
     * @param keyValuePairs alternating {@code String} keys and {@code Object}
     *                      values
