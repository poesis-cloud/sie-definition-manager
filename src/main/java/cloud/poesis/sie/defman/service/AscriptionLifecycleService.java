...
/**
 * Orchestrates GSM lifecycle transitions with referee preconditions, inter-transition cascades,
 * governance convergence, and identity-bound validation.
 *
 * <p>Sourced from {@code gsm-ascription-lifecycle} state machine diagram. Replaces inline lifecycle
 * logic formerly in {@link AscriptionService}.
 *
...
public class AscriptionLifecycleService {
  ...
  /**
   * Constructs the lifecycle service, building the cascade graph from all registered subtype
   * services.
   *
   * @param subtypeServices all {@link AbstractAscriptionService} implementations
  ...
  public AscriptionStatusTransitionEntity transition(UUID ascriptionId, String targetStatus) {
    ...
    try {
      ...
      //    Must run BEFORE recording the ACTIVE transition, because the DB
      //    trigger on the transition row sets the subtype table status
      //    synchronously — if the predecessor is still ACTIVE, the
      //    uq_{type}_active unique constraint fires.
      if (targetStatusType == AscriptionStatusType.ACTIVE) {
