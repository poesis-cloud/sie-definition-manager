...
public class MechanismRuleValidationService {
  ...
  String validateStarlarkRule(String rule) {
    ...
    for (Statement stmt : file.getStatements()) {
      validateDictLiteralConformance(stmt, resolvedArchetypes);
      if (stmt instanceof ForStatement fs) {
        for (Statement body : fs.getBody()) {
          validateDictLiteralConformance(body, resolvedArchetypes);
        }
      }
    }
  ...
  private String validateOnTrigger(StarlarkFile file) {
    ...
    if (receiveCall == null) {
      throw RuleViolationException.of(
          ...
          "Mechanism rule must begin with sys.receive(\"<Archetype URI>\") as its first executable statement",
          "field",
          "rule",
          "construct",
          "sys.receive");
    ...
    for (int i = 1; i < chain.size(); i++) {
      ...
      if (link.args().size() != 1) {
        throw RuleViolationException.of(
            ...
            "." + link.method() + "() requires exactly 1 argument (Archetype URI)",
            "method",
            link.method());
      }
      if (!(link.args().get(0).getValue() instanceof StringLiteral)) {
        throw RuleViolationException.of(
            AscriptionConsistencyRuleType.MECHANISM_RULE_SYS_FLUENT_API_ARITY,
            "." + link.method() + "() argument must be an Archetype URI string literal",
            "method",
            link.method());
      }
    }
  ...
  private void validateSysCallsInExpr(Expression expr) {
    ...
    if (parsingService.isSysEffectChain(call)) {
      ...
      if (effectArgs.isEmpty() || effectArgs.size() > 2) {
        throw RuleViolationException.of(
            AscriptionConsistencyRuleType.MECHANISM_RULE_SYS_FLUENT_API_ARITY,
            "sys.effect() requires 1-2 positional arguments (Archetype URI, optional data)",
            "method",
            "effect");
      }
      if (!(effectArgs.get(0).getValue() instanceof StringLiteral)) {
        throw RuleViolationException.of(
            AscriptionConsistencyRuleType.MECHANISM_RULE_SYS_FLUENT_API_ARITY,
            "sys.effect() first argument must be an Archetype URI string literal",
            "method",
            "effect");
      }
  ...
  // ======================================================================
  // sys.* Archetype identity collection (V10)
  // ======================================================================

  private void collectSysArchetypeIds(Statement stmt, Set<String> names) {
    if (stmt instanceof ExpressionStatement es) {
      collectSysArchetypeIdsInExpr(es.getExpression(), names);
    } else if (stmt instanceof AssignmentStatement as) {
      collectSysArchetypeIdsInExpr(as.getRHS(), names);
    }
  }
  ...
  private void validateDictLiteralConformance(
      Statement stmt, Map<String, ArchetypeEntity> resolvedArchetypes) {
    if (stmt instanceof ExpressionStatement es) {
      validateDictLiteralInSysCall(es.getExpression(), resolvedArchetypes);
    } else if (stmt instanceof AssignmentStatement as) {
      validateDictLiteralInSysCall(as.getRHS(), resolvedArchetypes);
    }
  }
