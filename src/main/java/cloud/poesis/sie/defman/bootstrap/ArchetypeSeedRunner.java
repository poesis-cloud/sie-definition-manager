 ...
 * are temporarily disabled so archetypes can be inserted with explicit {@code status=ACTIVE,
 * version=1}; lifecycle transitions are inserted for audit trail completeness.
 *
 * <p>Why disable the transition triggers:
 *
 * <ul>
 *   <li>{@code tgf_assign_ascription_version} would increment version on the APPROVED transition,
 *       corrupting the intended {@code version=1} to 2.
 *   <li>{@code tgf_sync_ascription_status} would redundantly walk the archetype row through
 *       DRAFT→PROPOSED→APPROVED→ACTIVE when we already set ACTIVE directly at INSERT time.
 * </ul>
 *
 * <p>Schema files are sourced from {@code definition/schemas/gsm-archetypes/} (mapped to classpath
 * via Maven resource directory). Single source of truth: edit schema files, restart — no migration
 * changes needed.
 *
