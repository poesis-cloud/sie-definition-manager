...
/**
 * Seeds GSM base archetypes from {@code gsm/schemas/*.json} classpath resources on first startup
 * (idempotent — skips if archetypes already exist).
 *
 * <p>Uses raw JDBC. IDs and timestamps are generated by DB defaults/triggers. The status-sync
 * trigger is temporarily disabled so archetypes can be inserted with explicit {@code status=ACTIVE}
 * and {@code version=1}; lifecycle transitions are inserted for audit trail completeness.
 *
 ...
 * INSERT time.
 *
 * <p>Schema files are sourced from {@code src/main/resources/gsm/schemas/} (a pinned snapshot of
 * the gsm-specifications repo). Single source of truth: edit schema files, restart — no migration
 * changes needed.
 *
