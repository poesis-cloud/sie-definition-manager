 ...
 * transition
 * triggers ({@code trg_ast_sync_ascription_status} and
 * {@code trg_ast_assign_ascription_version})
 * are temporarily disabled so archetypes can be inserted with explicit
 * {@code status=ACTIVE,
 ...
 * row through
 * DRAFT→PROPOSED→APPROVED→ACTIVE when we already set ACTIVE directly at INSERT
 * time.
 * </ul>
 *
 * <p>
 * Schema files are sourced from {@code def/schemas/gsm-archetypes/} (mapped to
 * classpath via
 * Maven resource directory). Single source of truth: edit schema files, restart
 * — no migration
 * changes needed.
 *
