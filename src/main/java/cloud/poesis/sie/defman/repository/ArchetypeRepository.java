...
public interface ArchetypeRepository extends AbstractAscriptionRepository<ArchetypeEntity> {
    ...
    /**
     * Resolves the single archetype row that this exact, version-pinned Archetype
     * URI dereferences
     * to.
     *
     * <p>
     * Resolvability is acquired <em>once</em>, at approval, when the governance
     * version is
     * materialized ({@code version > 0}), and is thereafter <em>permanent</em>: no
     * later status
     * change revokes it. So candidate URIs (DRAFT/PROPOSED) never resolve here —
     * and rival candidates
     * may legitimately share one URI — while every post-approval status resolves,
     * including {@code
     * DEPRECATED} and {@code RETIRED}, because statements are immutable and
     * existing {@code $ref}
     * chains must keep resolving. Resolution is therefore not an eligibility check:
     * callers needing
     * typing eligibility (ACTIVE/DEPRECATED) must additionally validate status
     * themselves (see {@code
     * ArchetypeService#validateTypingEligibility}). At most one row can ever match:
     * {@code
     * uq_archetype_resolvable_uri} DB-enforces global uniqueness of the resolvable
     * URI across all
     * archetypes.
     *
     ...
     */
    @Query(value = "SELECT * FROM archetype WHERE statement->>'$id' = :uri AND version > 0", nativeQuery = true)
    Optional<ArchetypeEntity> findResolvableByUri(@Param("uri") String uri);

    /**
     * Atomically acquires or reads the permanent owner of an Archetype identity
     * stem.
     *
     ...
     */
    @Query(value = "SELECT gsm_acquire_archetype_stem_owner(:stem, :definitionId)", nativeQuery = true)
