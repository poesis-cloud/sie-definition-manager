...
/**
  ...
  *
  * <p>The definition-manager is the sole write proxy for governed schemas. Consumers read directly
...
public class SchemaRegistryClient {
    ...
    public JsonNode getSchema(int schemaId) {
        ...
        try {
            return mapper.readTree(
                    Objects.requireNonNull(response.get("schema"), "Schema Registry response missing schema")
                            .asText());
        } catch (Exception e) {
    ...
    public String buildSchemaUri(int schemaId) {
        URI baseUri =
                restClient.options().uri("/").retrieve().toBodilessEntity().getHeaders().getLocation();
