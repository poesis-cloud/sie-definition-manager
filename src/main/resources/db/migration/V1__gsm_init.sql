-- ============================================================
-- V1__gsm_init.sql  –  GSM consolidated baseline schema (merged V1-V10)
--
...
--   8. version is materialized on approval and remains immutable thereafter.
...
CREATE TABLE IF NOT EXISTS archetype(
    ...
    CONSTRAINT ck_archetype_status_version CHECK(
        (
            status IN(
                'DRAFT',
                'PROPOSED',
...
CREATE TABLE IF NOT EXISTS archetype_stem_owner(
    stem text NOT NULL,
    definition_id uuid NOT NULL REFERENCES definition(id) ON
            DELETE RESTRICT,
    CONSTRAINT archetype_stem_owner_pk PRIMARY KEY(stem)
);
-- 3c. structure ---------------------------------------------------
CREATE TABLE IF NOT EXISTS STRUCTURE(
    ...
    status ascription_status NOT NULL DEFAULT 'DRAFT',
    version INTEGER NOT NULL DEFAULT 0,
    CONSTRAINT ck_structure_status_version CHECK(
        (
            status IN(
                'DRAFT',
                'PROPOSED',
...
CREATE TABLE IF NOT EXISTS mechanism(
    ...
    CONSTRAINT ck_mechanism_status_version CHECK(
        (
            status IN(
                'DRAFT',
                'PROPOSED',
...
CREATE TABLE IF NOT EXISTS effector(
    ...
    CONSTRAINT ck_effector_status_version CHECK(
        (
            status IN(
                'DRAFT',
                'PROPOSED',
...
CREATE TABLE IF NOT EXISTS receptor(
    ...
    CONSTRAINT ck_receptor_status_version CHECK(
        (
            status IN(
                'DRAFT',
                'PROPOSED',
...
CREATE TABLE IF NOT EXISTS interaction(
    ...
    CONSTRAINT ck_interaction_status_version CHECK(
        (
            status IN(
                'DRAFT',
                'PROPOSED',
...
CREATE TABLE IF NOT EXISTS directive(
    ...
    CONSTRAINT ck_directive_status_version CHECK(
        (
            status IN(
                'DRAFT',
                'PROPOSED',
...
CREATE TABLE IF NOT EXISTS norm(
    ...
    structure_id uuid NOT NULL REFERENCES STRUCTURE(id),
    qualifier_id uuid NOT NULL REFERENCES archetype(id),
    CONSTRAINT ck_norm_status_version CHECK(
        (
            status IN(
                'DRAFT',
                'PROPOSED',
...
CREATE INDEX IF NOT EXISTS ix_archetype_stem ON archetype(
    (
        regexp_replace(
            STATEMENT->>'$id',
            '/v[1-9][0-9]*$',
...
CREATE INDEX IF NOT EXISTS idx_directive_stmt_purpose ON directive(
    (STATEMENT->>'purpose')
);
...
CREATE INDEX IF NOT EXISTS idx_ast_latest_lookup ON ascription_status_transition(
    ascription_id,
    "timestamp" DESC,
    id DESC
);
CREATE INDEX IF NOT EXISTS idx_ast_edge ON ascription_status_transition(
    ascription_id,
    pre_status,
    post_status
);
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_archetype_definition_version ON archetype(definition_id, version)
WHERE version > 0;
CREATE UNIQUE INDEX IF NOT EXISTS uq_archetype_resolvable_uri ON archetype(
    (STATEMENT->>'$id')
)
WHERE version > 0;
CREATE INDEX IF NOT EXISTS ix_archetype_id ON archetype(
    (STATEMENT->>'$id')
);
...
WHERE status = 'APPROVED';
CREATE UNIQUE INDEX IF NOT EXISTS uq_structure_definition_version ON STRUCTURE(definition_id, version)
WHERE version > 0;
...
WHERE status = 'APPROVED';
CREATE UNIQUE INDEX IF NOT EXISTS uq_mechanism_definition_version ON mechanism(definition_id, version)
WHERE version > 0;
...
WHERE status = 'APPROVED';
CREATE UNIQUE INDEX IF NOT EXISTS uq_effector_definition_version ON effector(definition_id, version)
WHERE version > 0;
...
WHERE status = 'APPROVED';
CREATE UNIQUE INDEX IF NOT EXISTS uq_receptor_definition_version ON receptor(definition_id, version)
WHERE version > 0;
...
WHERE status = 'APPROVED';
CREATE UNIQUE INDEX IF NOT EXISTS uq_interaction_definition_version ON interaction(definition_id, version)
WHERE version > 0;
...
WHERE status = 'APPROVED';
CREATE UNIQUE INDEX IF NOT EXISTS uq_directive_definition_version ON directive(definition_id, version)
WHERE version > 0;
...
WHERE status = 'APPROVED';
CREATE UNIQUE INDEX IF NOT EXISTS uq_norm_definition_version ON norm(definition_id, version)
WHERE version > 0;
-- 4e. Expression indexes — GSM §9 identity uniqueness -------------
-- Structure.purpose globally unique among ACTIVE
CREATE UNIQUE INDEX IF NOT EXISTS uq_structure_purpose ON STRUCTURE(
    (STATEMENT->>'purpose')
)
WHERE status = 'ACTIVE';
-- Mechanism.function unique within owning Structure among ACTIVE
CREATE UNIQUE INDEX IF NOT EXISTS uq_mechanism_function ON mechanism(
    structure_id,
    (STATEMENT->>'function')
)
WHERE status = 'ACTIVE';
...
-- §6  Trigger functions
-- ============================================================
-- 6a. Acquire permanent Archetype stem ownership ---------------
CREATE OR replace FUNCTION gsm_acquire_archetype_stem_owner(
        p_stem text,
        p_definition_id uuid
    ) RETURNS uuid AS $$
DECLARE owner_definition_id uuid;
BEGIN PERFORM pg_advisory_xact_lock(
    hashtextextended('stem:' || p_stem, 0)
);
INSERT INTO archetype_stem_owner(
        stem,
        definition_id
    )
VALUES(p_stem, p_definition_id) ON CONFLICT(stem) DO
UPDATE
SET definition_id = archetype_stem_owner.definition_id
RETURNING definition_id INTO owner_definition_id;
RETURN owner_definition_id;
END;
$$ LANGUAGE plpgsql;
CREATE OR replace FUNCTION tgf_reject_archetype_stem_owner_mutation() RETURNS TRIGGER AS $$ BEGIN IF TG_OP = 'DELETE' THEN RAISE integrity_constraint_violation USING message = 'Archetype stem ownership is permanent',
    CONSTRAINT = 'ck_archetype_stem_owner_immutable';
END IF;
IF NEW.stem IS DISTINCT
FROM OLD.stem
    OR NEW.definition_id IS DISTINCT
FROM OLD.definition_id THEN RAISE integrity_constraint_violation USING message = 'Archetype stem ownership cannot be changed',
    CONSTRAINT = 'ck_archetype_stem_owner_immutable';
END IF;
RETURN NEW;
END;
...
END;
$$ LANGUAGE plpgsql;
-- 6f. Materialize Definition-scoped version on approval ----------
CREATE OR replace FUNCTION tgf_materialize_ascription_version() RETURNS TRIGGER AS $$
DECLARE next_version INTEGER;
BEGIN IF OLD.status <> 'APPROVED'
AND NEW.status = 'APPROVED' THEN EXECUTE format(
    'select coalesce(max(version), 0) + 1 from %I where definition_id = $1 and version > 0',
    TG_TABLE_NAME
) INTO next_version USING NEW.definition_id;
NEW.version := next_version;
ELSIF NEW.version IS DISTINCT
FROM OLD.version THEN raise EXCEPTION '% id=%: version is database-managed',
    TG_TABLE_NAME,
    OLD.id USING errcode = 'integrity_constraint_violation',
    CONSTRAINT = 'ck_' || TG_TABLE_NAME || '_version_writer';
END IF;
RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
CREATE TRIGGER trg_archetype_stem_owner_immutable BEFORE
UPDATE
    OR DELETE ON archetype_stem_owner FOR EACH ROW EXECUTE FUNCTION tgf_reject_archetype_stem_owner_mutation();
-- ---- archetype ----
...
CREATE TRIGGER trg_archetype_version_materialization BEFORE
UPDATE OF status,
    version ON archetype FOR EACH ROW EXECUTE FUNCTION tgf_materialize_ascription_version();
...
CREATE TRIGGER trg_structure_version_materialization BEFORE
UPDATE OF status,
    version ON STRUCTURE FOR EACH ROW EXECUTE FUNCTION tgf_materialize_ascription_version();
...
CREATE TRIGGER trg_mechanism_version_materialization BEFORE
UPDATE OF status,
    version ON mechanism FOR EACH ROW EXECUTE FUNCTION tgf_materialize_ascription_version();
...
CREATE TRIGGER trg_effector_version_materialization BEFORE
UPDATE OF status,
    version ON effector FOR EACH ROW EXECUTE FUNCTION tgf_materialize_ascription_version();
...
CREATE TRIGGER trg_receptor_version_materialization BEFORE
UPDATE OF status,
    version ON receptor FOR EACH ROW EXECUTE FUNCTION tgf_materialize_ascription_version();
...
CREATE TRIGGER trg_interaction_version_materialization BEFORE
UPDATE OF status,
    version ON interaction FOR EACH ROW EXECUTE FUNCTION tgf_materialize_ascription_version();
...
CREATE TRIGGER trg_directive_version_materialization BEFORE
UPDATE OF status,
    version ON directive FOR EACH ROW EXECUTE FUNCTION tgf_materialize_ascription_version();
...
CREATE TRIGGER trg_norm_version_materialization BEFORE
UPDATE OF status,
    version ON norm FOR EACH ROW EXECUTE FUNCTION tgf_materialize_ascription_version();
