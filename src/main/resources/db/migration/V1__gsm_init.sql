...
CREATE
    TABLE
        IF NOT EXISTS archetype_stem_owner(
            stem text NOT NULL,
...
CREATE
    INDEX IF NOT EXISTS ix_archetype_stem ON
    archetype(
        (
            regexp_replace(
                STATEMENT ->> '$id',
...
CREATE
    UNIQUE INDEX IF NOT EXISTS uq_archetype_definition_version ON
    archetype(
        definition_id,
        version
    )
...
CREATE
    UNIQUE INDEX IF NOT EXISTS uq_archetype_resolvable_uri ON
    archetype(
        (
            STATEMENT ->> '$id'
        )
...
CREATE
    INDEX IF NOT EXISTS ix_archetype_id ON
    archetype(
        (
            STATEMENT ->> '$id'
        )
...
CREATE
    UNIQUE INDEX IF NOT EXISTS uq_structure_definition_version ON
    STRUCTURE(
        definition_id,
        version
    )
...
CREATE
    UNIQUE INDEX IF NOT EXISTS uq_mechanism_definition_version ON
    mechanism(
        definition_id,
        version
    )
...
CREATE
    UNIQUE INDEX IF NOT EXISTS uq_effector_definition_version ON
    effector(
        definition_id,
        version
    )
...
CREATE
    UNIQUE INDEX IF NOT EXISTS uq_receptor_definition_version ON
    receptor(
        definition_id,
        version
    )
...
CREATE
    UNIQUE INDEX IF NOT EXISTS uq_interaction_definition_version ON
    interaction(
        definition_id,
        version
    )
...
CREATE
    UNIQUE INDEX IF NOT EXISTS uq_directive_definition_version ON
    directive(
        definition_id,
        version
    )
...
CREATE
    UNIQUE INDEX IF NOT EXISTS uq_norm_definition_version ON
    norm(
        definition_id,
        version
    )
...
CREATE
    OR replace FUNCTION gsm_acquire_archetype_stem_owner(
        p_stem text,
        p_definition_id uuid
    ) RETURNS uuid AS $$ DECLARE owner_definition_id uuid;
...
INSERT
    ...
    CONFLICT(stem) DO UPDATE
    SET
        definition_id = archetype_stem_owner.definition_id RETURNING definition_id INTO
            owner_definition_id;
...
CREATE
    OR replace FUNCTION tgf_reject_archetype_stem_owner_mutation() RETURNS TRIGGER AS $$ BEGIN IF TG_OP = 'DELETE' THEN RAISE integrity_constraint_violation
        USING message = 'Archetype stem ownership is permanent',
...
FROM
OLD.definition_id THEN RAISE integrity_constraint_violation
    USING message = 'Archetype stem ownership cannot be changed',
CONSTRAINT = 'ck_archetype_stem_owner_immutable';
END IF;
...
CREATE
    OR replace FUNCTION tgf_materialize_ascription_version() RETURNS TRIGGER AS $$ DECLARE next_version INTEGER;
...
AND NEW.status = 'APPROVED' THEN EXECUTE format(
    ...
    next_version
        USING NEW.definition_id;

NEW.version := next_version;

ELSIF NEW.version IS DISTINCT
FROM
OLD.version THEN raise EXCEPTION '% id=%: version is database-managed',
TG_TABLE_NAME,
OLD.id
...
CREATE
    TRIGGER trg_archetype_stem_owner_immutable BEFORE UPDATE
        OR DELETE
            ON
            archetype_stem_owner FOR EACH ROW EXECUTE FUNCTION tgf_reject_archetype_stem_owner_mutation();
...
CREATE
    TRIGGER trg_archetype_version_materialization BEFORE UPDATE
        OF status,
        version ON
        archetype FOR EACH ROW EXECUTE FUNCTION tgf_materialize_ascription_version();
...
CREATE
    TRIGGER trg_structure_version_materialization BEFORE UPDATE
        OF status,
        version ON
        STRUCTURE FOR EACH ROW EXECUTE FUNCTION tgf_materialize_ascription_version();
...
CREATE
    TRIGGER trg_mechanism_version_materialization BEFORE UPDATE
        OF status,
        version ON
        mechanism FOR EACH ROW EXECUTE FUNCTION tgf_materialize_ascription_version();
...
CREATE
    TRIGGER trg_effector_version_materialization BEFORE UPDATE
        OF status,
        version ON
        effector FOR EACH ROW EXECUTE FUNCTION tgf_materialize_ascription_version();
...
CREATE
    TRIGGER trg_receptor_version_materialization BEFORE UPDATE
        OF status,
        version ON
        receptor FOR EACH ROW EXECUTE FUNCTION tgf_materialize_ascription_version();
...
CREATE
    TRIGGER trg_interaction_version_materialization BEFORE UPDATE
        OF status,
        version ON
        interaction FOR EACH ROW EXECUTE FUNCTION tgf_materialize_ascription_version();
...
CREATE
    TRIGGER trg_directive_version_materialization BEFORE UPDATE
        OF status,
        version ON
        directive FOR EACH ROW EXECUTE FUNCTION tgf_materialize_ascription_version();
...
CREATE
    TRIGGER trg_norm_version_materialization BEFORE UPDATE
        OF status,
        version ON
        norm FOR EACH ROW EXECUTE FUNCTION tgf_materialize_ascription_version();
