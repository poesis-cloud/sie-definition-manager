...
class AscriptionResourceIT {
  ...
  void createArchetype_returnsDraftWithHalLinks() throws Exception {
    ...
    mvc.perform(get("/api/v1/ascriptions/{id}", createdArchetypeId))
        .andExpect(
            jsonPath("$._links.self.href", endsWith("/ascriptions/" + createdArchetypeId)))
        .andExpect(
            jsonPath(
        ...
        .andExpect(
            jsonPath(
                "$._links.type.href",
                endsWith("/definitions/" + seedArchetypeDefinitionId)))
        .andExpect(
            jsonPath(
  ...
  void createStructure_withArchetypeFk() throws Exception {
    ...
    mvc.perform(get("/api/v1/ascriptions/{id}", createdStructureId))
        .andExpect(
            jsonPath(
                "$._links.type.href",
                endsWith("/definitions/" + structureArchetypeDefinitionId)))
        .andExpect(
            jsonPath(
                "$._links.describedby.href",
  ...
  void createMechanism_withStructureFkInStatement() throws Exception {
    ...
    mvc.perform(get("/api/v1/ascriptions/{id}", createdMechanismId))
        .andExpect(
            jsonPath(
                "$._links.type.href",
                endsWith("/definitions/" + mechanismArchetypeDefinitionId)));
  }
  ...
  void getSchema_returnsComposedJsonSchema() throws Exception {
    // Create a fresh structure to test with — its typing archetype is StructureArchetype
    ...
    // The composed schema inlines the typing archetype's schema as the statement property.
