...
class AscriptionStatusTransitionServiceObservabilityTest {
    ...
    @BeforeEach
    void setUp() {
        Query advisoryLockQuery = mock(Query.class);
        lenient()
                .when(entityManager.createNativeQuery(any(String.class)))
                .thenReturn(advisoryLockQuery);
        lenient()
                .when(advisoryLockQuery.setParameter(any(String.class), any()))
                .thenReturn(advisoryLockQuery);
        lenient().when(advisoryLockQuery.getSingleResult()).thenReturn(null);
    ...
    void acceptedTransition_enrichesActiveSpanWithLifecycleAttributes() {
        ...
        AscriptionEntity entity = stubEntity(ascriptionId, DefinitionSubjectType.STRUCTURE, AscriptionStatusType.DRAFT);
    ...
    void acceptedTransition_emitsPersistenceEventOnly() {
        ...
        assertThat(transitionSpan.getEvents())
                .anyMatch(
                        event -> event.getName().equals(EVENT_HOOK_PERSISTENCE)
                                && "DRAFT"
                                        .equals(
                                                event
                                                        .getAttributes()
        ...
        assertThat(otel.getLogRecords())
                .noneMatch(
                        record -> EVENT_HOOK_PERSISTENCE.equals(
    ...
    void activationTransition_doesNotEmitDelegatedActivationHookEvent() {
        UUID ascriptionId = UUID.randomUUID();
        AscriptionEntity entity = stubEntity(ascriptionId, DefinitionSubjectType.STRUCTURE,
    ...
    void deactivationTransition_doesNotEmitDelegatedDeactivationHookEvent() {
        ...
        assertThat(otel.getLogRecords())
                .noneMatch(
                        record -> "gsm.ascription.hook.deactivation"
    ...
    void activationHandoff_emitsGovernanceEventForPreviousActiveSibling() {
        ...
        AscriptionEntity activating = stubEntity(activatingId, DefinitionSubjectType.STRUCTURE,
                AscriptionStatusType.APPROVED);
        AscriptionEntity previousActive = stubEntity(previousActiveId, DefinitionSubjectType.STRUCTURE,
                AscriptionStatusType.ACTIVE);
        ...
        assertThat(transitionSpan.getEvents())
                .anyMatch(
                        event -> event.getName().equals(EVENT_HOOK_ACTIVATION_HANDOFF)
                                && previousActiveId
                                        .toString()
                                        .equals(
        ...
        assertThat(otel.getLogRecords())
                .noneMatch(
                        record -> EVENT_HOOK_ACTIVATION_HANDOFF.equals(
                                record.getAttributes().get(AttributeKey.stringKey("event.name")))
                                && OUTCOME_SUCCESS.equals(
                                        record.getAttributes().get(AttributeKey.stringKey("event.outcome"))));
    }
    ...
    void approvalConvergence_emitsGovernanceEventsForTerminatedSiblings() {
        ...
        AscriptionEntity approved = stubEntity(approvedId, DefinitionSubjectType.STRUCTURE,
                AscriptionStatusType.PROPOSED);
        AscriptionEntity draftSibling = stubEntity(draftSiblingId, DefinitionSubjectType.STRUCTURE,
                AscriptionStatusType.DRAFT);
        AscriptionEntity proposedSibling = stubEntity(
        ...
        assertThat(transitionSpan.getEvents())
                .anyMatch(
                        event -> event.getName().equals(EVENT_HOOK_APPROVAL_CONVERGENCE)
                                && draftSiblingId
                                        .toString()
                                        .equals(
                                                event.getAttributes().get(AttributeKey.stringKey("gsm.ascription.id")))
        ...
        assertThat(otel.getLogRecords())
                .noneMatch(
                        record -> EVENT_HOOK_APPROVAL_CONVERGENCE.equals(
                                record.getAttributes().get(AttributeKey.stringKey("event.name")))
                                && OUTCOME_SUCCESS.equals(
                                        record.getAttributes().get(AttributeKey.stringKey("event.outcome"))));
    }
    ...
    void cascadedTransitions_emitCascadeEventsOnSingleActiveSpan() {
        ...
        AscriptionEntity source = stubEntity(sourceId, DefinitionSubjectType.STRUCTURE, AscriptionStatusType.DRAFT);
        AscriptionEntity mechanism = stubEntity(mechanismId, DefinitionSubjectType.MECHANISM,
                AscriptionStatusType.DRAFT);
        AscriptionEntity effector = stubEntity(effectorId, DefinitionSubjectType.EFFECTOR, AscriptionStatusType.DRAFT);
        ...
        assertThat(sourceTransition.getEvents())
                .anyMatch(
                        event -> event.getName().equals(EVENT_HOOK_CASCADE)
                                && "GOVERNING"
                                        .equals(
                                                event
                                                        .getAttributes()
    ...
    void dependentCascadeNotApplicable_emitsReasonAndWarnLogWithBody() {
        UUID sourceId = UUID.randomUUID();
        AscriptionEntity source = stubEntity(sourceId, DefinitionSubjectType.MECHANISM, AscriptionStatusType.PROPOSED);
        when(entityManager.find(AscriptionEntity.class, sourceId)).thenReturn(source);
        ...
        assertThat(transitionSpan.getEvents())
                .anyMatch(
                        event -> event.getName().equals(EVENT_HOOK_CASCADE_SKIP)
    ...
    void cascadeTargetFailure_propagatesWithoutErrorTelemetry() {
        ...
        assertThat(otel.getLogRecords())
                .noneMatch(
                        record -> EVENT_HOOK_ERROR.equals(
                                record.getAttributes().get(AttributeKey.stringKey("event.name"))));
    }
    ...
    void invalidTransition_propagatesWithoutErrorEvent() {
        ...
        SpanData transitionSpan = otel.getSpans().stream()
    ...
    private AscriptionStatusTransitionService createService(
        ...
        AscriptionStatusTransitionService configuredService = new AscriptionStatusTransitionService(
    ...
    private LogRecordData findLifecycleLogRecord(String eventName, String eventOutcome) {
        return otel.getLogRecords().stream()
                .filter(
                        record -> eventName.equals(record.getAttributes().get(AttributeKey.stringKey("event.name")))
                                && eventOutcome.equals(
                                        record.getAttributes().get(AttributeKey.stringKey("event.outcome"))))
                .findFirst()
                .orElseThrow();
