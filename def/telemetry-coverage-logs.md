...
| **JSON Log Rendering**                   | Logback               | Logback JSON encoder / OTel Logback appender  | Any log line                         | JSON log structure / OTLP LogRecord         | `required`           | `[OTLP path — S-006]` Log event bridged to OTLP exporter as a `LogRecord` via `OpenTelemetryAppender` (agent-installed in prod; test-installed in IT). Stdout JSON shape (`timestamp`, `level`, `logger`, `message`, ECS field names) `[stdout path — S-006b — pending]`. | OTLP path: `LogRecord` present at exporter. Stdout path: structured JSON fields per S-006b. | `OpenTelemetryAppenderIT#logEmittedViaSlf4jReachesOpenTelemetryExporter` (OTLP); stdout JSON validity test (S-006b — pending) | S-006 / S-006b     |
| **Log-Trace Correlation (In-Span)**      | Logback / OTel agent  | OTel Java agent + collector (resource attrs)  | Log line inside active span          | OTLP LogRecord trace/span fields            | `required`           | `[OTLP path — S-006]` `trace_id` / `span_id` set on `LogRecord` by the OTel agent (agent-owned per ADR-001 D-3 + ADR-003 D-4); Defman code MUST NOT write them (machine-checked). Stdout pattern carries no `%X{trace_id}` (R-INV-1). | `trace_id`, `span_id` on `LogRecord`; absent from Defman MDC writes                       | `SemConvGuardTest` (negative — code never writes); agent-emitted in prod | S-006              |
| **Log-Trace Correlation (No-Span)**      | Logback / OTel agent  | OTel Java agent (LogRecord)                   | Log line outside active span         | OTLP LogRecord trace/span fields absent     | `required`           | `[OTLP path — S-006]` `trace_id` / `span_id` absent from `LogRecord` attributes (absent ≠ empty, per R-INV-4). Same machine guard applies to stdout — keys never injected by code. | Fields not present in `LogRecord.getAttributes()`                                         | `OpenTelemetryAppenderIT#mdcAttributesAbsentWhenNotSet` (absent-key assertion) | S-006              |
| **Stdout JSON Encoding (ECS shape)**     | Logback               | Logback JSON encoder (stdout appender)        | Any log line when sink ∈ {stdout, both} | Structured JSON on stdout                | `required`           | `[stdout path — S-006b — pending]` Structured JSON encoding on stdout following ECS field names (`@timestamp`, `log.level`, `log.logger`, `message`, `service.name`-equivalents — final field set defined by S-006b). Distinct from the OTLP path which carries `LogRecord` natively. | ECS-aligned JSON envelope on stdout when sink ≠ `otlp`                                    | S-006b stdout JSON shape IT (pending — owned by S-006b)         | S-006b             |
| **Stdout JSON Omit-Absent MDC Keys**     | Logback               | Logback JSON encoder (stdout appender)        | Log line with MDC keys unset         | Stdout JSON key omission                    | `required`           | `[stdout path — S-006b — pending]` Absent MDC keys (`gsm.tenant.id`, `sie.component`) MUST be omitted from the stdout JSON envelope, not rendered as empty strings (mirrors R-INV-4 on the OTLP path). | Keys absent from JSON object when not set in MDC                                          | S-006b stdout JSON omit-absent IT (pending — owned by S-006b)   | S-006b             |
...
**Quantitative criterion (PRD-001 §4 / F-AC-8):** ≥95% of `required` units (denominator = non-AOP rows above) must have passing evidence. The AOP-covered entry/exit/exception log set is 100% structurally and is not part of the percentage denominator (the structural test green-light _is_ the evidence).
...
- **S-006 [OTLP path]:** OTLP `LogRecord` bridging via `OpenTelemetryAppender` (AC-2); MDC vocab on `LogRecord` attributes — present in-request, absent out-of-request (AC-3); stdout sink matrix (AC-4); sem-conv compliance machine guard (AC-5); AOP log-level binding preserved (AC-6); Helm `observability.logs.sink` enum (AC-1).
- **S-006b [stdout path — pending]:** Stdout JSON encoding (ECS field names); omit-absent MDC keys in the stdout JSON encoder.
...
| 2026-06-01 | S-006  | OTLP log export + dual-sink Helm switch landed (ADR-003 D-2). Three pre-existing rows (`JSON Log Rendering`, `Log-Trace Correlation (In-Span)`, `Log-Trace Correlation (No-Span)`) tagged `[OTLP path — S-006]` in the Expected Telemetry column — row identifiers preserved verbatim per Q2 NO-RENAME lock. Owner/Layer columns updated to reflect that `trace_id` / `span_id` are now agent-owned on the `LogRecord` (ADR-001 D-3, ADR-003 D-4). Two new rows added with `[stdout path — S-006b — pending]` for the stdout JSON encoding (ECS shape) and omit-absent MDC behaviour that S-006b will deliver. Routing Table split S-006 into a `[OTLP path]` block and S-006b into a `[stdout path — pending]` block. Evidence on the OTLP side: 9/9 ITs green (`OpenTelemetryAppenderIT` 5/5 + `LogsSinkSwitchIT` 4/4) + `SemConvGuardTest` 3/3 + Helm `helm-template-matrix` workflow (3 positive cells + 1 negative case) all PASS. |
